...
客户端对比夫服务器要简单一点只需要connect一下，但是服务器需要注册监听套接字，然后bind端口等信息，然后再listen监听套接字情况，accept是一个阻塞函数

### 网络处理

这里处理网络等内容我采用epoll处理并发设计，如果对于简单的任务处理其实可以采用多线程来处理，但是这里为了学习reactor模型，我会结合epoll+线程池实现reactor模型来处理，同时在离线情况下的处理很简单，只需要确保qt上位机和播放器同时可以处理就好了，难点在于后续引入服务器之后的请求转发以及服务器--->播放器的音频数据处理很难，首先引入线程池处理离线模型下的任务
线程池的简易实现可以参考 `https://github.com/Shanxidysq/Thread_pool`，自己参考别人实现的线程池

### 网络通信问题

和上位机通信采用socket编程，例如我们在单片机开发中可能会使用到mqtt协议，这里可以采用socket作为底层传输协议，然后使用json和cjson作为协议栈，具体选型等等还是需要我们去操作的，当然了这里涉及到网络编程模型之后，就不会再是简单的cin输入选型，而是通过协议栈的数据收发，然后通过状态机解析命令，或者协议解析获取之后执行相应的操作。

这里在通信协议的选择上我才用cjson，json我们还需要自己移植等操作，采用cjson简化了移植操作

### Qt上位机

qt制作的上位机很简单，只需要我们进行简单的创建socket连接，然后发送消息码就可以了，后面我会整理一下整个项目的代码，引入服务器作为推流的部分（当然这部分内容需要学习ffmpeg+音视频编解码等知识后做），后面会完善整个项目
